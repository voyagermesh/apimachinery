...
func (r Ingress) BackendServices() map[string]metav1.ObjectMeta {
	...
	if r.Spec.DefaultBackend != nil {
		record(r.Spec.DefaultBackend.Service.Name)
	}
	for _, rule := range r.Spec.Rules {
		if rule.HTTP != nil {
			for _, svc := range rule.HTTP.Paths {
				record(svc.Backend.Service.Name)
			}
		} else if rule.TCP != nil {
			record(rule.TCP.Backend.Service.Name)
		}
	}
...
func (r Ingress) HasBackendService(name, namespace string) bool {
	...
	if r.Spec.DefaultBackend != nil {
		if fqn(r.Spec.DefaultBackend.Service.Name) == svcFQN {
			return true
		}
	}
	for _, rule := range r.Spec.Rules {
		if rule.HTTP != nil {
			for _, svc := range rule.HTTP.Paths {
				if fqn(svc.Backend.Service.Name) == svcFQN {
					return true
				}
			}
		} else if rule.TCP != nil {
			if fqn(rule.TCP.Backend.Service.Name) == svcFQN {
				return true
			}
		}
	}
...
func (r Ingress) ProtoWithALPN() error {
	...
	if r.Spec.DefaultBackend != nil {
		if r.Spec.DefaultBackend.Proto != "" && len(r.Spec.DefaultBackend.ALPN) > 0 {
...
func (r Ingress) UseHTX() bool {
	// check default backend
	if r.Spec.DefaultBackend != nil {
		if containsH2(r.Spec.DefaultBackend.Proto, r.Spec.DefaultBackend.ALPN) {
			return true
		}
	}
	for _, rule := range r.Spec.Rules {
