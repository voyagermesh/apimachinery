...
func (r Ingress) IsValid(cloudProvider string) error {
	...
	for ri, rule := range r.Spec.FrontendRules {
		if _, err := checkRequiredPort(rule.Port); err != nil {
			return errors.Errorf("spec.frontendRules[%d].port %d is invalid. Reason: %s", ri, rule.Port, err)
		}
	}
	...
	for ri, rule := range r.Spec.Rules {
		if rule.HTTP != nil && rule.TCP == nil {
			...
			if err != nil {
				return errors.Errorf("spec.rules[%d].http.port %d is invalid. Reason: %s", ri, rule.HTTP.Port, err)
			}
			...
			if nodePort, err = checkOptionalPort(rule.HTTP.NodePort); err != nil {
				return errors.Errorf("spec.rules[%d].http.nodePort %d is invalid. Reason: %s", ri, rule.HTTP.NodePort, err)
			} else if nodePort > 0 {
				if r.LBType() == LBTypeHostPort || r.LBType() == LBTypeInternal {
					return errors.Errorf("spec.rules[%d].http.nodePort %d may not be specified when `LBType` is %s", ri, rule.HTTP.NodePort, r.LBType())
				}
			}
			...
			for pi, path := range rule.HTTP.Paths {
				...
				if !checkBackendServiceName(path.Backend.Service.Name) {
					return errors.Errorf("spec.rules[%d].http.paths[%d] has invalid serviceName for addr %s and path %s", ri, pi, a, path.Path)
				}
				if errs := validation.IsDNS1123Subdomain(path.Backend.Service.Name); len(errs) > 0 {
			...
			if podPort, err = checkRequiredPort(rule.TCP.Port); err != nil {
				return errors.Errorf("spec.rules[%d].tcp.port %d is invalid. Reason: %s", ri, rule.TCP.Port, err)
			}
			if nodePort, err = checkOptionalPort(rule.TCP.NodePort); err != nil {
				return errors.Errorf("spec.rules[%d].tcp.nodePort %d is invalid. Reason: %s", ri, rule.TCP.NodePort, err)
			} else if nodePort > 0 {
				if r.LBType() == LBTypeHostPort || r.LBType() == LBTypeInternal {
					return errors.Errorf("spec.rules[%d].tcp.nodePort %d may not be specified when `LBType` is %s", ri, rule.TCP.NodePort, r.LBType())
				}
			}
			...
			if !checkBackendServiceName(rule.TCP.Backend.Service.Name) {
				return errors.Errorf("spec.rules[%d].tcp has invalid serviceName for addr %s", ri, a)
			}
			if errs := validation.IsDNS1123Subdomain(rule.TCP.Backend.Service.Name); len(errs) > 0 {
	...
	if !usesHTTPRule && r.Spec.DefaultBackend != nil {
		if !checkBackendServiceName(r.Spec.DefaultBackend.Service.Name) {
		...
		if rule.Auth != nil && rule.Auth.OAuth != nil {
			...
			for ii, oauth := range rule.Auth.OAuth {
				...
				if jj, found := oauthHosts[oauth.Host]; found {
					return errors.Errorf("spec.frontendRules[%d].oauth[%d] is reusing host %s for port %d, also used in spec.frontendRules[%d].oauth[%d]", ri, ii, rule.Auth.OAuth[ii].Host, rule.Port, ri, jj)
				} else {
...
func checkRequiredPort(port int32) (int, error) {
	if port <= 0 {
		return 0, errors.Errorf("port %d must a +ve integer", port)
	}
	return int(port), nil
}

func checkOptionalPort(port int32) (int, error) {
	if port < 0 {
		return 0, errors.Errorf("port %d can't be -ve integer", port)
	}
	return int(port), nil
}
