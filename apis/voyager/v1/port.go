	...
	for _, rule := range r.Spec.Rules {
		if rule.HTTP != nil {
			...
			if r.UseTLSForRule(rule) {
				if port := rule.HTTP.Port; port > 0 {
					mappings[int(port)] = Target{PodPort: int(port), NodePort: int(rule.HTTP.NodePort)}
				} else {
					mappings[443] = Target{PodPort: 443, NodePort: int(rule.HTTP.NodePort)}
				}
			} else {
				if port := rule.HTTP.Port; port > 0 {
					mappings[int(port)] = Target{PodPort: int(port), NodePort: int(rule.HTTP.NodePort)}
				} else {
					mappings[80] = Target{PodPort: 80, NodePort: int(rule.HTTP.NodePort)}
				}
			}
		} else if rule.TCP != nil {
			mappings[int(rule.TCP.Port)] = Target{
				PodPort:  int(rule.TCP.Port),
				NodePort: int(rule.TCP.NodePort),
			}
		}
	}
