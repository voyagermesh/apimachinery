...
SRC_PKGS := apis client crds hack # directories which hold app source excluding tests (not vendored)
SRC_DIRS := $(SRC_PKGS)
...
CONVERSION_GROUPS ?= voyager:v1beta1
...
add-license:
	...
	@docker run --rm 	                                 \
		...
		--env HTTPS_PROXY=$(HTTPS_PROXY)                 \
		$(BUILD_IMAGE)                                   \
		ltag -t "./hack/license" --excludes ".go vendor contrib libbuild third_party" -v
...
check-license:
	...
	@docker run --rm 	                                 \
		...
		--env HTTPS_PROXY=$(HTTPS_PROXY)                 \
		$(BUILD_IMAGE)                                   \
		ltag -t "./hack/license" --excludes ".go vendor contrib libbuild third_party" --check -v
